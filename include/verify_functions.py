...
def verify_turing_machine(states, input_alphabet, tape_alphabet, delta, start, final_states):
    ok = True
    ...
    if not tape_alphabet:
        print("Error: The tape alphabet is empty")
        ok = False
    if len(tape_alphabet) != len(set(tape_alphabet)):
        print("Error: The tape alphabet contains duplicates")
        ok = False
    ...
    for line in delta:
        if len(line) != 5:
            ...
            if line[0] not in states:
                print(f"Error: State {line[0]} not in the machine's states")
                ok = False
            if line[1] not in tape_alphabet:
                print(f"Error: Read symbol {line[1]} not in the tape alphabet")
                ok = False
            if line[2] not in states:
                print(f"Error: Next state {line[2]} not in the machine's states")
                ok = False
            if line[3] not in tape_alphabet:
                print(f"Error: Write symbol {line[3]} not in the tape alphabet")
                ok = False
            if line[4] not in ("L", "R"):
                print(f"Error: Invalid direction {line[4]} (must be 'L' or 'R')")
                ok = False
